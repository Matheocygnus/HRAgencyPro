...
// Invoice routes
  ...
  // Check Airtable connection and list available tables
  app.get("/api/airtable/check", hasRole(["super_admin", "admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/airtable/:tableName", hasRole(["super_admin", "admin"]), async (req, res) => {
    try {
      const { tableName } = req.params;
      const records = await getRecordsFromTable(tableName);
      res.json({ records });
    } catch (error: any) {
      console.error(`Error getting records from Airtable table ${req.params.tableName}:`, error);
      res.status(500).json({ 
        message: `Failed to get records from table ${req.params.tableName}`, 
        error: error.message 
      });
    }
  });
  
  // Import prospects and heroes from pasted data
  app.post("/api/airtable/import", hasRole(["super_admin", "admin"]), async (req, res) => {
    try {
      const { data } = req.body;
      ...
      const people = processPastedData(data);
      
      // Filter out entries without roles
      const peopleWithRoles = people.filter(person => person.role);
      
      // Import into Airtable
      const results = await importPeopleToAirtable(peopleWithRoles);
      
      // Return success response
      res.json({ 
        message: "Data imported successfully", 
        results,
        totalProcessed: peopleWithRoles.length
      });
    } catch (error: any) {
      console.error("Error importing data to Airtable:", error);
      res.status(500).json({ 
        message: "Failed to import data to Airtable", 
        error: error.message 
      });
    }
  });
  
  // Synchronize Airtable Heroes with database
  app.post("/api/airtable/sync/heroes", hasRole(["super_admin", "admin"]), async (req, res) => {
    try {
      const heroes = await getRecordsFromTable('Heroes');
      let importedCount = 0;
      
      for (const hero of heroes) {
        try {
          // Map Airtable fields to your database schema
          const heroData = {
            firstName: hero['First Name'],
            lastName: hero['Last Name'],
            email: hero['Email'],
            phone: hero['Phone'] || '',
            position: hero['Role'] || '',
            status: 'Active',
            notes: `Imported from Airtable - ${new Date().toISOString()}`
          };
          
          await storage.createHero(heroData);
          importedCount++;
        } catch (err) {
          console.error(`Error importing hero ${hero['Full Name']}:`, err);
        }
      }
      
      res.json({ 
        message: `Successfully synchronized ${importedCount} heroes from Airtable`,
        totalProcessed: heroes.length,
        imported: importedCount
      });
    } catch (error: any) {
      console.error("Error synchronizing heroes from Airtable:", error);
      res.status(500).json({ 
        message: "Failed to synchronize heroes from Airtable", 
        error: error.message 
      });
    }
  });
  
  // Synchronize Airtable Prospects with database
  app.post("/api/airtable/sync/prospects", hasRole(["super_admin", "admin"]), async (req, res) => {
    try {
      const prospects = await getRecordsFromTable('Prospects');
      let importedCount = 0;
      ...
      for (const prospect of prospects) {
        try {
          ...
          console.error(`Error importing prospect ${prospect['Full Name']}:`, err);
        }
      }
      
      res.json({ 
        message: `Successfully synchronized ${importedCount} prospects from Airtable`,
        totalProcessed: prospects.length,
        imported: importedCount
      });
    } catch (error: any) {
      console.error("Error synchronizing prospects from Airtable:", error);
      res.status(500).json({ 
        message: "Failed to synchronize prospects from Airtable", 
        error: error.message 
      });
    }
  });
