...
export default function SidebarDev({ isMobileOpen, setMobileOpen }: { 
  isMobileOpen: boolean; 
  setMobileOpen: (open: boolean) => void;
}) {
  const [location] = useLocation();
  
  // In dev mode, we assume the user is a super admin
  const isAdmin = true;
    ...
    <>
      ...
      <aside className={sidebarClasses}>
        ...
        <nav className="p-4">
          ...
          <div className="mt-8 text-xs text-white bg-gray-700 bg-opacity-50 p-2 rounded-md text-center">
            Development Mode
          </div>
        </nav>
      </aside>
